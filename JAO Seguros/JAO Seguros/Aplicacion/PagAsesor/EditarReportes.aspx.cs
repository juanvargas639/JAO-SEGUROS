...
namespace JAO_Seguros.Aplicacion.PagAsesor
{
    public partial class EditarReportes : System.Web.UI.Page
    {
        ...
        protected void btnEditarReporte_Click(object sender, EventArgs e)
        {
            ...
            {
                ScriptManager.RegisterClientScriptBlock(this.Page, this.Page.GetType(), "alert", "swal({title: 'Error!',text: 'No se pudo agregar la respuesta al reporte, inténtalo más tarde', icon: 'error', timer: 1500,button: false}).then(function() { },function(dismiss) {if (dismiss === 'timer'){console.log('I was closed by the timer')}})", true);
            }
        }
    }
}