...
namespace JAO_Seguros.Aplicacion.PagAdmin
{
    public partial class Polizas : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    List<Poliza> listaPoliza = gestionPoliza.listadoPoliza();
                    TablaPolizasActivas.DataSource = listaPoliza;
                    TablaPolizasActivas.DataBind();
                    // Requerido para que funcione jQuery DataTables.
                    TablaPolizasActivas.UseAccessibleHeader = true;
                    TablaPolizasActivas.HeaderRow.TableSection = TableRowSection.TableHeader;
                }
            } else
            {
                /*--RESTRINGIR AL USUARIO SI ACCEDE A LA PÁGINA SIN LOGUEO--*/
        ...
        protected void lnkBtnEdit_Click(object sender, EventArgs e)
        {
            ...
            string id = Convert.ToString(TablaPolizasActivas.DataKeys[gvRow.RowIndex].Value);
